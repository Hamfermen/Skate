        ...
        if data:
            try:
                ...
                if isinstance(data, bytes):
                    data = (
                        data.decode().strip()
                    )  # Decode the bytes to a string and remove extra spaces
...
for i in range(1, 7):
    btn = ctk.CTkButton(
        profile_frame, text=str(i), width=button_size[0], height=button_size[1]
    )
    btn.bind(
        "<Button-1>", lambda event, i=i: on_press(f"d{i}")
...
for i in range(1, 6):
    btn = ctk.CTkButton(
        size_frame, text=str(i), width=button_size[0], height=button_size[1]
    )
    btn.bind(
        "<Button-1>", lambda event, i=i: on_press(f"p{i}")
    )  # Send profile selection
...
clamp_stop_button = ctk.CTkButton(
    position_frame, text="Clamp Stop", width=button_size[0], height=button_size[1]
)
...
clamp_open_button = ctk.CTkButton(
    position_frame,
    text="Clamp Open",
    fg_color="orange",
    width=button_size[0],
...
# X -0.2mm
x_neg_button = ctk.CTkButton(
    movement_frame, text="X -0.2mm", width=button_size[0], height=button_size[1]
)
...
# Y -0.2mm
y_neg_button = ctk.CTkButton(
    movement_frame, text="Y -0.2mm", width=button_size[0], height=button_size[1]
)
